...
stratified by the five-level radiologist subtlety rating, with an image-level
cluster bootstrap interval on every figure.

Bone suppression, proposed but not tested by earlier stratified work on this
dataset, raises aggregate CPM reproducibly: $+2.9$ $[+0.7, +5.1]$ across five
seeds under ten-fold, and $+4.3$ $[+0.7, +8.6]$ under leave-one-out. \emph{Where}
that gain sits does not replicate --- ten-fold attributes it to the
second-easiest stratum, leave-one-out to the second-hardest. We report this
rather than resolve it: at 12--50 nodules per stratum, a per-stratum estimate
cannot carry a mechanistic claim.

Establishing even that aggregate effect required separating training from
sampling uncertainty, which are routinely conflated: a single-run bootstrap gives
$[-0.9, +6.3]$ and hides it, because roughly four CPM points of run-to-run spread
from random initialisation are absorbed into what that interval presents as
sampling error.
...
the detector reaches 16.5\% $[14.7, 18.3]$ against 43.3\% internally.

This is a study of evaluation methodology, not a proposed clinical system, with
JSRT as its case study. We claim no novelty for stratified evaluation or for the
frozen-backbone observation, both reported previously.
...
This is a paper about how detection performance is measured, not a proposal
for a clinical system. JSRT serves as a case study rather than a target: it is
small, single-source and single-nodule, but it is the one public chest-radiograph
dataset carrying a validated per-nodule difficulty scale, which is what makes the
questions we ask answerable at all. The procedures we describe --- stratified
reporting, image-level intervals, separation of training from sampling noise ---
apply to any detection study, and their consequences are most visible, not least
valid, at this sample size.
...
from the released scores without retraining. Two rows of Table~\ref{tab:loocv} come from earlier runs that used a single
seed and did not retain per-image scores; they are labelled as such, and no claim
rests on them.
...
ResNet-34 rows come from earlier single-seed runs whose per-image scores were not
kept, so no interval can be computed for them retrospectively.}
\label{tab:loocv}
\begin{tabular}{llc}
\toprule
...
\midrule
\multicolumn{3}{l}{\emph{Earlier single-seed runs, scores not retained:}} \\
...
from the same earlier single-seed runs as Table~\ref{tab:loocv}, for which no
interval is available. Level 1 is the most subtle. Note the small denominators: at
...
reporting renders invisible in either direction. Those two tables include single-seed runs without intervals, from which we
draw no inference; the same phenomenon appears in
...
\begin{enumerate}
  ...
  \item \textbf{Performance below published work.} Our absolute CPM values are
        ...
        retained every stratum and evaluate up to 4 FP/image. The figures are
        therefore not comparable term for term, and the comparison illustrates
        the paper's point rather than settling it: an aggregate sensitivity is
        defined partly by which difficulty strata are admitted, so two numbers
        from different strata sets measure different quantities. A gap in
        absolute performance between us remains, and we do not claim otherwise.
        We report
...
\todo{BEFORE SUBMISSION: make the repository public, or replace the sentence below with ``available from the corresponding author on reasonable request''. The repository is currently private, so the sentence as written is false.}
...
\todo{BEFORE SUBMISSION: both authors must write and confirm this statement themselves. Suggested draft, to be corrected to fact: ``B.V.X. designed the study, implemented the detection and evaluation pipeline, ran the experiments and drafted the manuscript. T.T.Q.N. contributed to the experimental design and the statistical analysis, and revised the manuscript. Both authors read and approved the final manuscript.''}
