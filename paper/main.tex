...
Four results follow. First, stratification reverses the ordering suggested by
...
Fourth, we test rather than assert the usual explanation for why data
augmentation degrades a frozen-backbone detector. Augmentation costs 22.1 CPM
points with the backbone frozen; unfreezing four transformer blocks reduces that
penalty to 3.1 points with an interval containing zero, supporting the account
that the damage comes from the backbone's inability to adapt rather than from
augmentation itself.
