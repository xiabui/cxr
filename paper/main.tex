...
\newcommand{\pending}[1]{\textcolor{orange}{\textbf{[CHỜ KẾT QUẢ: #1]}}}
...
\author{\todo{tên tác giả, đơn vị, email}}
\date{}
...
\todo{2--3 câu về các hướng phát hiện nốt trên X-quang ngực, kèm trích dẫn}
...
\paragraph{JSRT.}
The JSRT dataset contains 247 posteroanterior chest radiographs: 154 with a
...
112{,}120 images were used to pre-train the ResNet-34 backbone. Labels in this
...
241 paired radiographs with and without bone shadow \todo{cite nguồn Kaggle}
...
\begin{description}
  ...
  \item[Bone suppression + attention.] The same encoder with a second input
        ...
        the paired data, plus CBAM attention blocks.
...
over six false-positive rates. \pending{khoảng tin cậy cho ba dòng này —
chạy \texttt{bootstrap\_ci.py} trên scores LOOCV}}
\label{tab:loocv}
\begin{tabular}{lcc}
\toprule
...
\pending{bảng trung bình ± độ lệch chuẩn qua 5 seed cho cả hai nhánh —
\texttt{run\_seeds.py}, đang chạy}
...
\pending{external validation trên VinDr-CXR — xem mục Thảo luận}
...
our experiments do not test. \pending{kết quả tinh chỉnh một phần sẽ kiểm chứng
trực tiếp: nếu mở băng vài tầng cuối làm tăng cường hết phản tác dụng thì giả
thuyết đúng; nếu không thì phải viết lại đoạn này}

\paragraph{Relation to prior findings.}
Our observation that frozen foundation features lose small-lesion signal
reproduces a result published independently \todo{cite bài 2026}. We regard the
...
\pending{kết quả VinDr-CXR}. We note in advance that the hit criterion on
VinDr-CXR differs from that used on JSRT: VinDr annotations are bounding boxes
...
\todo{link repo}. Bootstrap and multi-seed tooling
